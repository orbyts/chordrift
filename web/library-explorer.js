(function (root) {
  const text = (value) => String(value == null ? '' : value).toLocaleLowerCase();
  const time = (value) => {
  ...
  const stable = (tracks, compare) => tracks
  ...
  function sortPlaylistTracks(tracks, mode) {
    const compare = {
      custom_order: (a, b) => (a.position || 0) - (b.position || 0),
      most_played: (a, b) => (b.play_count || 0) - (a.play_count || 0) || compareText(a.title, b.title),
    ...
    }[mode] || ((a, b) => (a.position || 0) - (b.position || 0));
  ...
  function lastHeardBucket(lastPlayedAt, now = Date.now()) {
    ...
    const lastHeard = new Date(lastPlayedAt).getTime();
    if (!Number.isFinite(lastHeard)) return 'Never heard in retained history';
    const ageDays = Math.max(0, now - lastHeard) / 86_400_000;
