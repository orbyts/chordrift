...
| Beta.4 → beta.5 | Safari had no Chordrift product-session cookie, so opening a fresh CLI authorization URL returned a bare 401 with no sign-in continuation. Chrome did have a session and the server accepted consent with 303, but Safe Browsing intercepted the loopback redirect before the installed CLI received it. | An unauthenticated trusted browser carries the already validated loopback PKCE request through Google/Chordrift login and returns to the same one-time consent page. The continuation is restricted to the internal CLI authorization path and cannot redirect externally. Chrome reputation handling remains independent; Safari is the explicit trusted-browser acceptance path while review is pending. | Unit regressions round-trip every CLI authorization parameter and reject absolute, scheme-relative, unrelated, fragment-bearing, and header-injection continuation targets. Release acceptance must show Safari sign-in, consent, loopback delivery, token exchange, credential storage, and compatibility negotiation. |
| Beta.5 → beta.6 | Safari completed Auth0, returned to the correct consent page, and Chordrift accepted the first click, but Safari did not navigate from the HTTPS approval response to the live `127.0.0.1` listener. Replaying the consumed form produced an empty response Safari downloaded as `approve`. | New CLIs request polling completion. After one explicit authenticated consent, the browser displays a terminal success page while the CLI polls Chordrift for the single-use code. PKCE still binds exchange to the initiating CLI; approval, poll result, and exchange expire together. Older loopback clients remain compatible. | Tests require a 32-byte opaque polling state, preserve it through product login, reject malformed states, and run the full transport/security suite. Release acceptance must complete from a Safari consent without any localhost browser navigation or downloaded response. |
| Beta.6 → beta.7 | A portable CLI could authenticate to Chordrift, but its only named Spotify authorization command was the legacy local transport that stored the provider refresh credential on that machine. | Product identity and provider consent are independent hosted grants. The remote CLI opens a server-derived Spotify route, never receives a provider token, and polls the same account-scoped connection DTO as web until encrypted hosted readiness is visible. A missing browser product session returns through Auth0 only to the exact internal provider route. | Parser and continuation tests reject external/malformed targets; full CI covers the shared provider query contract. Release acceptance installs on a fresh machine, authorizes Chordrift, then authorizes Spotify as two separate commands. |
| Beta.7 → beta.8 | A newly installed CLI on a second Mac opened Chrome's Safe Browsing interstitial specifically for Chordrift's custom `/auth/cli/approve` page, even though ordinary Web UI Auth0 login on the same origin was clean. The custom loopback/PKCE/poll protocol had recreated a standards problem already solved for native clients. | Replace the entire custom CLI browser flow with Auth0's OAuth 2.0 Device Authorization Flow using a separate public Native application. The default browser opens Auth0's verification URI; Auth0 owns login, consent, expiry, and polling cadence. Chordrift validates the resulting issuer credential and mints the same account-scoped, revocable machine session. Remove the Chordrift approval page, synchronizer token, custom status endpoint, and localhost callback. | A permanent fake Auth0/service harness proves discovery, short-code presentation, `authorization_pending`, prescribed polling, issuer-origin enforcement, identity exchange, and a distinct product session without OS credentials or localhost browser navigation. Chrome, Safari, and Firefox use the same default-browser URL; `--no-open` remains the headless fallback. |
| Beta.9 → beta.10 | The remote CLI correctly negotiated contract 1.6, but the embedded browser wrapper still submitted hard-coded 1.5 and failed closed as incompatible after deployment. | Render the browser version from Rust's authoritative `CONTRACT_VERSION` instead of maintaining a second literal. | A server-rendering regression requires the delivered JavaScript to contain the exact Rust major/minor and no unresolved version placeholder; the final authenticated browser smoke must enable maintenance before release acceptance. |
| Beta.13 → beta.14 | Spotify and Neon contained two new tracks in the configured `Inbox`, but hosted maintenance returned `in sync` with zero observed changes. | Treat discovery provenance and canonical destination as independent facts. Carry every configured provider-native intake source into the shared Rust projection; Liked Songs remains the primary convenience intake, while Inbox, From Friends, Liked from Radio, From Prompts, and future configured sources remain valid provenance. | Unit regressions cover one named Inbox addition and a composite named-source-plus-Like event. A read-only live audit identifies the exact two Inbox tracks as current intake without a Spotify write. |
| Beta.14 → beta.15 | Two Inbox tracks received approved destinations, but immediate post-write observations lagged, exhausted fast retries as `recoverable`, and named intake had no provider cleanup effect. | Never clear intake before destination membership is observed. Retry read-only verification across a bounded eventual-consistency window; once placement is visible, create a separate exact review and batch named-playlist removals against one evolving provider snapshot. | The stateful fake-provider matrix now covers two Inbox tracks, two destinations, a stale post-write observation, a separate composite cleanup review, and final empty Inbox with both destinations intact. Projection and interpreter unit tests require named intake to become `remove_track`, not an Unlike. |
| Beta.15 → beta.16 | The two destinations became provider-visible while both tracks remained in Inbox. The immutable planner correctly emitted two deferred `cleanup/remove_track` operations, but the hosted PostgreSQL interpreter rejected that valid operation kind as `state_conflict` before showing the cleanup review. | A planner-owned consumed-intake removal is an expected bridge input, not conflicting provider drift. The PostgreSQL adapter must accept it, let the intake audit produce the single contract cleanup representation, and never duplicate it as an observed removal. | Pure adapter-boundary regression requires cleanup `remove_track` to pass the allowlist, be excluded from ordinary plan annotations, and reappear exactly once as `ConsumeIntake(Inbox)`. The convergence matrix permanently names already-placed-plus-source-present as a normal source-only cleanup state. |
| Post-beta.16 | Neon rejected database work after the project exceeded its quota. Auth0 still succeeded, but the callback collapsed the identity-store failure into HTTP 403 and the worker entered a restart storm. | Dependency availability is not authorization. Identity-backed routes preserve structured 503 responses for database outages, and the worker remains alive with bounded exponential backoff across connection, startup-query, and queue failures. | Unit regressions distinguish dependency-unavailable from permission-denied and prove bounded/resettable backoff plus retry classification. The incident requires no provider write. |
