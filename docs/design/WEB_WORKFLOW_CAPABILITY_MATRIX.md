...
| Saved/Liked intake | Detect new, rediscovered and already-placed tracks; remember keep/consume decisions; clear only after destination intent | Ask only when policy is unresolved | Optional exact saved-state change | Rust composition implemented; apply remains gated |
