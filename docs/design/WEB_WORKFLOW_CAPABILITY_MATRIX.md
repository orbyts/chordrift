...
| Listening summary | Query one immutable evidence batch, rank its recent observations, and report per-signal provenance, freshness and availability | Show a small head plus paginated **Show more**; distinguish recent API observations from archive-backed skips/completions | None | Planned, non-critical; provider-capability design recorded |
