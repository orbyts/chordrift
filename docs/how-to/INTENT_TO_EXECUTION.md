...
This is the plain-language map for the released v0.1.4 safety workflow. Use it
when an output mentions a proposal, batch, plan, assessment, apply run, or
snapshot and you want to know what that object means and whether Spotify has
changed yet.
...
The released commands above are routed through the same Rust application
facade being developed for v0.2.0. V0.2 may add new client-facing commands, but
the existing IDs, phase boundaries, and safety meanings remain compatibility
requirements for the CLI and future native clients.
